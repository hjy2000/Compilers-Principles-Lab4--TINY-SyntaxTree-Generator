...
typedef enum
{
	START, INASSIGN, INCOMMENT, INNUM, INID,INGT,INLT,INSUB,DONE //新增比较状态
}
StateType;
	...
	while (state != DONE)
	{
		...
		case START:
			if (isdigit(c))
				...
				state = INASSIGN;//这里为了方便直接把==这个比较运算符放到了assign这里
			else if (c == '>')
				state = INGT;//新增两个比较状态
			else if (c == '<')
				state = INLT;
